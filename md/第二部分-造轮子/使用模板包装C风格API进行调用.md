...
struct X {
    ...
    X(Fn&& func, Args&&... args) {
        ...
        // 注意这里make_unique创建的是堆上的std::tuple，而不是args，所以不会造成double-free
        // auto Decay_copied = std::make_unique<Tuple>(std::forward<Fn>(func), std::forward<Args>(args)...);
        auto Decay_copied = new Tuple(std::forward<Fn>(func), std::forward<Args>(args)...);
        auto Invoker_proc = gen<Tuple>(std::make_index_sequence<1 + sizeof...(Args)>{});
        f(Invoker_proc, Decay_copied);
    }
private:
    template <typename Tuple, std::size_t... Indices>
    static constexpr auto gen(std::index_sequence<Indices...>) noexcept {
        // 这个函数的唯一作用就是利用编译期序列来生成一个获取std::tuple中内容的工具函数，用作传入f的cb参数
        return &Invoke<Tuple, Indices...>;
    }

    template <typename Tuple, std::size_t... Indices>
    static unsigned int Invoke(void* RawVals) noexcept { // 这个签名得和cb_t一样
        // const std::unique_ptr<Tuple> FnVals(static_cast<Tuple*>(RawVals)); // 这个指针的唯一用处就是RAII自动释放资源，改成手动new/delete也是可以的
        // Tuple& Tup = *FnVals.get();
        Tuple* pTup = static_cast<Tuple*>(RawVals);
        Tuple& Tup = *pTup;
        std::invoke(std::move(std::get<Indices>(Tup))...); // 这里第一个参数就是func，编译期序列用于展平std::tuple
        delete pTup;
        return 0;
    }
};
...
3. `auto Invoker_proc = start<Tuple>(std::make_index_sequence<1 + sizeof...(Args)>{});` 调用静态成员函数模板 `start` 得到一个普通函数指针。这里需要详细展开。传递了类型参数 `Tuple` ，已经使用 `std::make_index_sequence` 制造了一个可变参数序列，用来遍历元组。
   ...
   将模板参数转发给 `Invoke` 进行实例化，获取这个生成的静态成员函数模板的地址，也就是符合签名 `cb_t` 的函数指针了。
   ...
   我们的 `Invoke` 利用模板生成代码，使用 `std::invoke` 支持了所有可调用类型，以及利用编译期序列展开+ `std::get` 来将 `std::tuple` 展平为实参序列。**无非是把 `void*` 指针转换为正确的形参类型再去使用罢了，而这个“正确的类型”，通过模板Tuple传递。**最终的调用，在 `std::invoke(std::move(std::get<Indices>(Tup))...);` 这里强制移动移动，目的是表达移动语义，和 `std::thread` 一样。
...
我们还可以将 `func` 的形参类型改为 `const int&` ，这可以通过编译，因为 `const int&` 可以引用右值表达式。当然了，打印的**地址不同**。原因也很简单，我们说了，智能指针通过 `decay` 后的 `tuple` 拷贝了一份，因此实际上**存储的是参数副本**，元组类型是使用 [`std::decay_t`](https://zh.cppreference.com/w/cpp/types/decay) 删除了 CV 与引用限定的。
