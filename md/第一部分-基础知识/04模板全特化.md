...
template<typename T>
constexpr bool is_void_v = is_void<T>::value; // 变量模板是全局的
...
template<>
constexpr const char* nameof<void> = "void";

template<>
constexpr const char* nameof<int> = "int";

int main(){
    std::cout << nameof<void> << '\n'; // void
    std::cout << nameof<int> << '\n';  // int
    std::cout << nameof<char> << '\n'; // ??
}
```

语法形式和前面函数模板、类模板都类似，很简单，这个变量模板是类型形参。我们特化了变量模板 `nameof` 的模板实参为 `void` 与 `int` 的情况，修改 `nameof` 的初始化器，让它的值不同。
...
**特化必须在导致隐式实例化的首次使用之前**，在**每个发生这种使用的翻译单元**中声明：
...
template<> // 错误 f<int> 的显式特化在隐式实例化之后出现（这会造成ODR）
...
为了保证<u>用户实现</u>的高度自定义，函数模板和变量模板的显式特化是否为 [inline](https://zh.cppreference.com/w/cpp/language/inline)/[constexpr](https://zh.cppreference.com/w/cpp/language/constexpr)/[constinit](https://zh.cppreference.com/w/cpp/language/constinit)/[consteval](https://zh.cppreference.com/w/cpp/language/consteval) **只与显式特化自身有关**，**主模板的声明是否带有对应说明符对它没有影响（==主模板的修饰只能影响到编译器隐式实例化的版本==）**。**但是模板声明中出现的[属性说明符](https://zh.cppreference.com/w/cpp/language/attributes)在它的显式特化是有效果的**：

```cpp
template<typename T>
int f(T) { return 6; } // 模板没有用任何特殊的东西修饰
template<>
constexpr int f<int>(int) { return 6; }   // OK，f<int> 是以 constexpr 修饰的

template<typename T>
[[nodiscard]] constexpr T g(T) { return 6; }  // 这里声明的 constexpr 修饰函数模板不会影响到特化版本，但是属性说明符会
template<>
int g<int>(int) { return 6; }             //OK，g<int> 不是以 constexpr 修饰的

int main(){
    constexpr auto n = f<int>(0);         // OK，f<int> 是显式实例化的版本，携带constexpr，可编译期求值
    //constexpr auto n2 = f<double>(0);   // Error! f<double> 不可编译期求值，因为其是隐式实例化的版本，没有携带constexpr

    //constexpr auto n3 = g<int>(0);      // Error! g<int> 不可编译期求值，因为其是显式实例化的版本，没有携带constexpr

    constexpr auto n4 = g<double>(0);     // OK! g<double> 是隐式实例化的版本，携带constexpr，可编译期求值
}
// 上面的实例化结果实际上是：
template<typename T>
int f(T) { return 6; }
...
template<>
inline constexpr int f<int>(int) { return 6; }

template<typename T>
[[nodiscard("")]] inline constexpr T g(T) { return 6; }

template<>
[[nodiscard("")]] inline constexpr double g<double>(double) { return 6; }

template<>
[[nodiscard("")]] int g<int>(int) { return 6; }
```
...
其实**就是注意有几层那就多套几个 `template<>`，并且指明模板类的模板实参**。

下面这样：就是自定义了 `X<void>` 且 `f<double>` 的情况下的函数。
