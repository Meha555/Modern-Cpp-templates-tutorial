...
```cpp
template<typename T>
const std::string categoryof = typeid(T).name();            // 主模板

template<typename T>
const std::string categoryof<T*> = std::format("pointer of {}", categoryof<T>); // 对指针的偏特化

template<typename T>
const std::string categoryof<T&> = std::format("lval-ref of {}", categoryof<T>); // 对左值引用的偏特化

template<typename T>
const std::string categoryof<T&&> = std::format("rval-ref of {}", categoryof<T>); // 对右值引用的偏特化

template<typename T>
const std::string categoryof<T[]> = std::format("array of {}", categoryof<T>); // 偏特化，但是只是对 T[] 这一类类型，而不是数组类型，因为 int[] 和 int[N] 不是一个类型

template<typename T, std::size_t N>
const std::string categoryof<T[N]> = std::format("array[{}] of {}", N, categoryof<T>); // 对T[N]的偏特化

std::cout << categoryof<int> << '\n';            // int
std::cout << categoryof<int*> << '\n';           // pointer of int
std::cout << categoryof<const int&&> << '\n';    // rval-ref of int
std::cout << categoryof<int[]> << '\n';          // array of int
std::cout << categoryof<int[1]> << '\n';         // array[1] of int
std::cout << categoryof<int[1][2]> << '\n';      // array[1] of array[2] of int
```

语法就是正常写主模板那样，然后在定义这个 `categoryof` 的时候，指明模板实参。或者你也可以定义非类型的模板形参的模板，偏特化，都是一样的写法。

不过与全特化不同，全特化不会写 `template<typename T>`，它是直接 `template<>`，然后指明具体的模板实参。

它与全特化最大的不同在于，全特化基本必写 `template<>`，而且定义的时候（如 `categoryof`）是指明具体的类型，而不是一类类型（`T*`、`T[]`）。
...
template<>
template<typename T2>
struct X<int, 10>::Y<int, T2> {     // 对全特化 X<int,10> 情况下偏特化的 Y<int>
    void f()const{}
};
...
int main(){
    ...
    y2.f();                     // Error 没有函数f()! 主模板模板实参不对
    X<int, 10>::Y<void, int>y3;
    y3.f();                     // Error 没有函数f()！成员函数模板模板实参不对
}
```
