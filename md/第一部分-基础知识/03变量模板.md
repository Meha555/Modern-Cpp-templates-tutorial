...
变<u>量模板实例化后简单的说**就是一个全局变量**，所以也不用担心生存期的问题</u>。
...
```cpp
std::cout << std::is_same_v<decltype(v<int>),const int>; // 1
```
...
**与函数模板和类模板不同，即使模板形参有默认实参，依然要求写明 `<>` （表明这是一个模板实例而非某个变量）。**
...
或者在 C++17 以 inline 或者 constexpr 修饰。**因为 C++17 规定了 inline 修饰静态数据成员，那么这就是在类内定义，不再需要类外定义。constexpr 在 C++17 修饰静态数据成员的时候，蕴含了 inline。**
