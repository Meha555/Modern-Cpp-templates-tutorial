...
我们**必须显式的指明类模板的类型实参，并且没有办法推导**，事实上这个空类在这里本身没什么意义。
...
> [c++11-17 模板核心知识（十二）—— 模板的模板参数 - 知乎](https://zhuanlan.zhihu.com/p/338915581)

类模板的模板类型形参可以接受一个类模板作为参数，我们将它称为：模板模板形参。

```cpp
// 假设我们有一个容器类模板，如果要指定其底层容器，就不得不这样：Stack<int, std::vector<int>>，发现int写了2遍
template <typename T, typename Cont = std::deque<T>>
class Stack;
// 但是改用模板模板形参，就可以让 Cont 的类型也自行推导：Stack<int, std::vector> 即可
template <typename T, typename <template E> Cont = std::deque> // 注意这里的写法
class Stack;
```
...
1. **可以有名字的模板模板形参**。
    ...
    Array<int, my_array> arr;    // arr 保有的成员是my_array<int> 而它保有了 int arr[10]
    ```
...
Array<int> arr; //arr的类型是 Array<int>，持有my_array<int>，其又持有int[1]的数组
```
